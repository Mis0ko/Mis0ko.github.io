...
- Abuser de la relation de confiance d'un hostname `loopback` ou `127.0.0.1` : `http://localhost/admin`
- 
...
Certaines applications bloquent les entrée contenant des noms d'hôtes comme `127.0.0.1` ou `localhost`, ou les URls sensibles comme `admin`. 
...
Parfois, une application ne place qu'un nom d'hôte ou une partie d'un chemin d'URL dans les paramètres de requête. La valeur soumise est ensuite concaténé dans une URL complète côté serveur avant qu'elle ne soit requêté.  
